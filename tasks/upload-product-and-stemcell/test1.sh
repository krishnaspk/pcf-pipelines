...
#cf login -a "${CF_API}" -u "${CF_USERNAME}" -p "${CF_PASSWORD}" -o "${CF_ORG}" -s Demo
cf login -a api.syslab.pcflabcog.com -u admin -p HZ-h6Uy1VFRZ1f7e3-Ba2S_-Axqq8JbP -o Demo -s Demo
cf target -s Demo
echo "$CFspace"
